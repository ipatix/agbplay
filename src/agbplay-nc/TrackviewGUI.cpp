...
void TrackviewGUI::update()
{
    ...
    for (uint32_t i = 0, th = 0; i < ntrks; i++, th += 2) {
        ...
        wattrset(
            winPtr,
            COLOR_PAIR(static_cast<int>(player.tracks[i].isMuted ? Color::TRK_NUM_MUTED : Color::TRK_NUM)) | aFlag
        );
        ...
        wattrset(
            winPtr,
            (trk.isCalling ? COLOR_PAIR(static_cast<int>(Color::TRK_LOC_CALL))
                           : COLOR_PAIR(static_cast<int>(Color::TRK_LOC)))
        );
...
#define DECIDE_COL_E(a, b)                            \
    ...
    : a && !b ? static_cast<int>(Color::TRK_FGW_BGC)  \
        ...
        for (size_t j = 0;; j++) {
            // C and C#
            wattrset(winPtr, COLOR_PAIR(DECIDE_COL_C(trk.activeNotes[j * 12 + 0], trk.activeNotes[j * 12 + 1])));
            waddstr(winPtr, "\u259D");

            // D and D#
            wattrset(winPtr, COLOR_PAIR(DECIDE_COL_D(trk.activeNotes[j * 12 + 2], trk.activeNotes[j * 12 + 3])));
            waddstr(winPtr, "\u259D");

            // E and F
            wattrset(winPtr, COLOR_PAIR(DECIDE_COL_E(trk.activeNotes[j * 12 + 4], trk.activeNotes[j * 12 + 5])));
            waddstr(winPtr, "\u2590");

            // F# and G
            wattrset(winPtr, COLOR_PAIR(DECIDE_COL_D(trk.activeNotes[j * 12 + 7], trk.activeNotes[j * 12 + 6])));
            waddstr(winPtr, "\u2598");
            ...
            // G# and A
            wattrset(winPtr, COLOR_PAIR(DECIDE_COL_D(trk.activeNotes[j * 12 + 9], trk.activeNotes[j * 12 + 8])));
            waddstr(winPtr, "\u2598");

            // A# and B
            wattrset(winPtr, COLOR_PAIR(DECIDE_COL_D(trk.activeNotes[j * 12 + 11], trk.activeNotes[j * 12 + 10])));
            waddstr(winPtr, "\u2598");
        }
...
#undef DECIDE_COL_E
        ...
        wattrset(
            winPtr,
            COLOR_PAIR(static_cast<int>(!trk.isMuted ? Color::TRK_LOUDNESS : Color::TRK_LOUDNESS_MUTED)) | A_REVERSE
        );
