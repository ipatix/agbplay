...
// Unsupported OS
#error \
    "Apparently your OS is neither Windows nor appears to be a UNIX variant (no unistd.h). You will have to add support for your OS in src/OS.cpp :/"
#endif
...
void OS::CheckTerminal()
{
    ...
    do {
        ...
        if (strncmp(pathbuf, "/Applications/Utilities/Terminal.app", 37) == 0) {
            std::cout << "It appears that you are using macOS's built-in Terminal.app." << std::endl
                      << std::endl
                      ...
                      << "run agbplay without any lag." << std::endl
                      << std::endl
